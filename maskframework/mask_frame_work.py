...
class Wafer(core.Elements):
    ...
    def __init__(self, size=4, draw=True, shape='disk'):
        ...
        if shape == 'disk':
            ...
            self.usableArea = 0.98*np.sqrt(2)*self.radius
    ...
    def addGridAlignementMarks(self):
        ...
        for i in range(int(self.x_max_index*(self.y_max_index) - 6)):
            ...
            if self.x_offset != 0:
                ...
                self.placeDesign(
                    am, (-self.x_spacing/2.+self.x_spacing, self.y_spacing/2.), N=1)
            ...
            if self.y_offset != 0:
                ...
                self.placeDesign(
                    am, (-self.x_spacing/2., self.y_spacing/2.-self.y_spacing), N=1)
            ...
            if self.x_offset != 0 and self.y_offset != 0:
                ...
                self.placeDesign(
                    am, (-self.x_spacing/2.+self.x_spacing, self.y_spacing/2.-self.y_spacing), N=1)
    ...
    def placeDesign(self, design, offset=(0, 0), N=1):
        ...
        d = ((self.x_spacing+self.x_offset)*(self.x_index-self.x_max_index/2+1/2.) +
             offset[0], -(self.y_spacing+self.y_offset)*(self.y_index-self.y_max_index/2+1/2.) + offset[1])
...
class AlignementMarkFillingBase():

    __metaclass__ = ABCMeta

    @abstractmethod
    def apply_filling(self, boundary, size):
        pass


class Null_Filling(AlignementMarkFillingBase):
...
@export
class Diffractive_Filling(AlignementMarkFillingBase):

    def __init__(self, spacing=600e-3, orientation='vertical'):
        # orientation: vertical and horizontal are allowed.

        self.spacing = spacing
        self.orientation = orientation
        super(Diffractive_Filling, self).__init__()
    ...
    def apply_filling(self, boundary, size):
        ...
        if self.orientation == 'vertical':
            ...
            for i in range(int(np.floor(w/self.spacing))):

                rect = shapes.Rectangle(
                    (-w/2. + i*self.spacing - self.spacing/4., -h/2.), (-w/2. + i*self.spacing + self.spacing/4., h/2.))

                boundary = substraction(boundary, rect)

        return boundary


class AlignementMarkBase():
    """Base class for alignementmarks"""

    __metaclass__ = ABCMeta

    def generate_alignement_mark(self):
        pass


@export
class AlignementMarkCross(AlignementMarkBase):

    def __init__(self, a=200, t=20, inverse=True, filling=Null_Filling()):

        self.inverse = inverse
        self.a = a  # edge length of the alignement mark
        self.t = t  # thickness of the cross
        self.filling = filling

        super(AlignementMarkCross, self).__init__()
        self.mark = self.generate_alignement_mark()

    def generate_alignement_mark(self):

        rect = shapes.Rectangle(
            (-self.a/2., -self.a/2.), (self.a/2., self.a/2.))
        ...
        if self.inverse == True:
            ...
            cross = substraction(rect, cross)

        cross = self.filling.apply_filling(cross, (a, a))

        return cross


@export
class AreaAlignBase(core.Elements):
    """Base class for a rectangle with alignement marks"""

    def __init__(self, width, height):
        self.width = width
        self.height = height
        super(AreaAlignBase, self).__init__()

    def addAlignementMarks(self, alignement_mark):

        self.add(utils.translate(alignement_mark,
...
@export
class PatternContainer(core.Elements):
    ...
    def __init__(self, description_only=False, draw=True, width=7000, height=6000):
        ...
        self.width = width
        self.height = height
        self.patternArea_x = width
        self.patternArea_y = height
        if draw:
            DrawArea = Rectangle(self.width, self.height)
            self.add(DrawArea)
    ...
    def addDescription(self, text, location='top'):
        ...
        if self.description_only:
            ...
            self.add(shapes.Label(
                text, 100, (-self.patternArea_x/2. - 3000, self.patternArea_y/2. + 2000)))
    ...
    def addPattern(self, shape='lines', radius=0, x_size=0, x_spacing=0, y_size=0, y_spacing=0):
        """creates a Pattern of the desired shape in the pattern container"""

        if shape == 'lines':

            self.addDescription('LINES X' + str(x_size) + 'Y' + str(y_size) +
                                'XSP' + str(x_spacing) + 'YSP' + str(y_spacing))
            ...
            if not self.description_only:
                ...
                if (y_spacing != 0):
                    ...
                    for i in range(y_span+1):
                        rect = shapes.Rectangle(
                            (-self.patternArea_x/2., -y_size/2.), (self.patternArea_x/2., y_size/2.))
                ...
                if (x_spacing != 0):
                    ...
                    for i in range(x_span+1):
                        rect = shapes.Rectangle(
                            (-x_size/2., -self.patternArea_y/2.), (x_size/2., self.patternArea_y/2.))
                        d = (i*x_spacing-self.patternArea_x/2., 0)
                        self.add(utils.translate(rect, d))

        if shape == 'grid':

            self.addDescription('GRID X' + str(x_size) + 'Y' + str(y_size) +
                                'XSP' + str(x_spacing) + 'YSP' + str(y_spacing))
        ...
        if shape == 'dots':
            ...
            self.addDescription('DOTS R' + str(radius) +
                                'XSP' + str(x_spacing) + 'YSP' + str(y_spacing))
            ...
            if not self.description_only:
                ...
                for i in range(y_span+1):
                    for j in range(x_span+1):
                        ...
                        d = (j*x_spacing-self.patternArea_x/2.,
                             i*y_spacing-self.patternArea_y/2.)
                        self.add(utils.translate(rect, d))

        if shape == 'rectangles':

            self.addDescription('SQUA X' + str(x_size) + 'Y' + str(y_size) +
                                'XSP' + str(x_spacing) + 'YSP' + str(y_spacing))
            ...
            if not self.description_only:
                ...
                for i in range(y_span+1):
                    for j in range(x_span+1):
                        rect = shapes.Rectangle(
                            (-x_size/2., -y_size/2.), (x_size/2., y_size/2.))
                        d = (j*x_spacing-self.patternArea_x/2.,
                             i*y_spacing-self.patternArea_y/2.)
                        self.add(utils.translate(rect, d))
...
class layer(core.Cell):
    ...
    def alignment_mark(self, a=5, t=0.1, center=(0, 0)):
        ...
        boundary = [[0, 0], [0, a/2.0], [a*t, a/2.0],
                    [a*t, a*t], [a/2.0, a*t], [a/2.0, 0]]
...
def test_area_align():
    ...
    filling = Diffractive_Filling()
    alignement_mark = AlignementMarkCross(filling=filling, inverse=True)
    layout = core.Layout('LIBRARY', unit=1e-06, precision=1e-09)
    cell = core.Cell('TOP')
    area = AreaAlignBase(1000, 1000)
    area.addAlignementMarks(alignement_mark.mark)
    cell.add(area)
    layout.add(cell)
    layout.save('Alignement_mark_test.gds')


def test_alignement_mark():
    # tests the basic alignement mark with the diffractive filling

    filling = Diffractive_Filling()
    alignement_mark = AlignementMarkCross(filling=filling, inverse=True)
    layout = core.Layout('LIBRARY', unit=1e-06, precision=1e-09)
    cell = core.Cell('TOP')
    container = core.Elements()
    container.add(alignement_mark.mark)
    cell.add(container)
    layout.add(cell)
    layout.save('Alignement_mark_test.gds')
